...
type RootHostRuntimeContext = DurableHostRuntimeContext & {
  ...
  return defineChannel<undefined, EveEventContext>({
    ...
    routes: [
      ...
      POST(EVE_SESSION_ROUTE_PATH, async (req, args) => {
        ...
        if (tokenRejection !== null) return await rejectHostRuntime(hostRuntime, tokenRejection);
        ...
        if (policyRejection !== null) return await rejectHostRuntime(hostRuntime, policyRejection);

        if (body.operationId !== undefined && forwarded.auth.principalType === "anonymous") {
          return await rejectHostRuntime(
            hostRuntime,
            Response.json(
        ...
        if (messageResult instanceof Response)
          return await rejectHostRuntime(hostRuntime, messageResult);
...
): Promise<RootHostRuntimeContext | Response | undefined> {
  const trusted = readTrustedHostRuntime(auth);
  if (trusted === undefined) return undefined;

  let prior;
  try {
    prior = await beginHostRuntimeAcceptance(trusted.acceptanceKey);
  } catch (error) {
    if (error instanceof HostRuntimeAcceptanceIndeterminateError) {
      return acceptanceStateUnavailableResponse();
    }
    throw error;
  }
  if (prior !== undefined) {
    if (prior === "INDETERMINATE") return acceptanceStateUnavailableResponse();
    return Response.json(
      {
        code: "HOST_RUNTIME_REFERENCE_INVALID",
        error: "The host runtime acceptance key was already used.",
        ok: false,
      },
      { headers: { "cache-control": "no-store" }, status: 409 },
    );
...
async function acceptHostRuntime(hostRuntime: RootHostRuntimeContext | undefined): Promise<void> {
...
async function rejectHostRuntime(
  hostRuntime: RootHostRuntimeContext | undefined,
  response: Response,
): Promise<Response> {
  if (hostRuntime !== undefined) {
...
function acceptanceStateUnavailableResponse(): Response {
  return Response.json(
    {
      code: "acceptance_state_unavailable",
      error: "The acceptance state is temporarily unavailable.",
