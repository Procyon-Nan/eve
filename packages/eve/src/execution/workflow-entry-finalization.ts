...
export async function finalizeExpiredSession(input: {
  ...
  const serializedContext = await settleRootHostRuntime({
    outcome: "cancelled",
    serializedContext: input.serializedContext,
  });
  ...
  if (input.mode === "task") {
    await fireSessionCallbackStep({ output: "", serializedContext, status: "completed" });
...
export async function finalizeDone(input: {
  ...
  if (input.mode === "task") {
    ...
    const settled: { isError?: boolean; output: unknown; usage?: TokenUsage } = {
      output,
      usage: input.action.usageDelta,
    };
    if (failed) settled.isError = true;
    await notifyTurnCallerStep({
      caller: input.caller,
      lifecycle: "terminal",
