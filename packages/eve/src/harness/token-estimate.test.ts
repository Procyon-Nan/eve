...
function userImage(image: unknown): ModelMessage {
  return {
    content: [{ image, mediaType: "image/png", type: "image" }],
    role: "user",
  } as ModelMessage;
}
...
describe("estimateModelMessageTokens", () => {
  ...
  it("does not push a roughly 3 MiB image near a 256K context threshold", () => {
    expect(estimateModelMessageTokens([userFile(Buffer.alloc(2_927_949))])).toBeLessThan(
      256_000 * 0.9,
    );
  });

  it("uses the same bounded reserve for image parts", () => {
    const bytes = Buffer.alloc(2_927_949, 0x61);
    const base64 = bytes.toString("base64");
    const estimates = [
      bytes,
    ...
    ].map((image) => estimateModelMessageTokens([userImage(image)]));

    expect(Math.max(...estimates)).toBeLessThan(4_000);
    expect(Math.min(...estimates)).toBeGreaterThanOrEqual(1_024);
  });
