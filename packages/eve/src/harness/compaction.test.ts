...
describe("compactMessages: active multimodal turn protection", () => {
  ...
  it("summarizes old history while preserving the current multimodal message", async () => {
    const oldUser = user("old investigation ".repeat(2_000));
    const messages = [oldUser, assistant("old result"), multimodalUser];
  ...
  it("recognizes multipart user input behind framework-generated string messages", async () => {
    const messages = [
      user("old question"),
  ...
  it("keeps the active multimodal turn ahead of a persistent-agent announcement", async () => {
    const toolCall: ModelMessage = {
    ...
    const announcement = user(`${AGENTS_SNIPPET_LABEL}\n<agents></agents>`);
    const activeTurn = [multimodalUser, toolCall, toolResult, announcement];
    const messages = [
