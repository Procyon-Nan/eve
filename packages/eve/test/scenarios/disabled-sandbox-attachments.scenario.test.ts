...
describe("disabled sandbox attachment runtime", () => {
  it(
    "passes image and PDF parts through a real dev server without provisioning a sandbox",
    async () => {
      ...
      try {
        ...
        const resumed = await sendDevelopmentMessage({
          message:
            "Call inspect_attachments again and report the files retained in session history.",
          serverUrl: server.url,
          session: result.session,
        });
        const resumedCompleted = readCompletedMessages(resumed.events);
        expect(resumedCompleted).toContain("data:image/png;base64,");
        expect(resumedCompleted).toContain("data:application/pdf;base64,");
        expect(resumedCompleted).not.toContain("eve-sandbox:");
