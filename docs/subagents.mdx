...
A server host can supply a specialist's model, business instructions, and ordinary dynamic tools as one locked runtime snapshot. Declare the provider on both `defineDynamic` and the returned agent so eve can reject an unauthorized specialist before its resolver runs:
...
Host-runtime specialists are turn-scoped, one-shot local subagents. They cannot use `experimental.subagentPersistentSessions`, select a separate model or compaction model, or delegate to another host-runtime specialist. The provider authorizes names from the top-level root snapshot and eve checks the authorization again immediately before creating the child reference.

The specialist directory still owns its static safety instructions, sandbox choice, connections, and other authored slots. Use `hostRuntimeInstructions` and `hostRuntimeTools` in dynamic instruction and tool slots to consume the host snapshot without exposing its opaque reference. The built-in `agent` copy reuses the root Turn's snapshot but receives no specialist authorization and does not own a separate release.
...
| Sandbox      | Shared with parent            | Own configured/disabled choice, else framework default |
...
For a declared subagent this means duplicating anything the child needs. When two subagents need the same procedure, copy the markdown under each `skills/` directory, or share typed helpers via `lib/`. The sandbox choice does not inherit from the parent; it falls back to the framework default unless the subagent authors `subagents/<id>/sandbox.ts`, explicitly calls `disableSandbox()` there, or seeds files via `subagents/<id>/sandbox/workspace/`.
