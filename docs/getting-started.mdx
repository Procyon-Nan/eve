...
| `sandbox.ts` or `sandbox/sandbox.ts`                    | The agent's sandbox selection             | Yes       | Use `sandbox.ts` for `defineSandbox(...)` or `disableSandbox()`; use `sandbox/sandbox.ts` with `sandbox/workspace/**` to seed files. The framework default applies when neither is authored. |
