...
File payloads must be base64 strings — raw bytes (`Uint8Array`, `Buffer`) are rejected because they do not survive eve's durable JSON boundary. eve persists the content output in the tool result, then creates a temporary model-call projection that keeps the tool call/result pair intact and presents the files as provider-neutral user `FilePart`s after the complete tool-result message. This temporary user message is not written to session history and does not emit `message.received`. OpenAI Chat Completions therefore receives file content as multimodal input rather than base64 JSON in a `tool` role message.

Keep payloads small: a content-part image is persisted in session history and re-projected on every subsequent model call, and eve warns above 3 MiB. Sending image parts to a model without vision support fails with that provider's error, the same as image parts in user messages.
