...
- Inline image and PDF `FilePart`s remain in the message and go directly to the configured model. eve does not write another copy or replace the payload with an `eve-sandbox:` URL.
