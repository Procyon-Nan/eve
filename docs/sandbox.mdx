...
The sandbox is the agent's isolated bash environment: a filesystem rooted at `/workspace` where it can run shell commands, execute scripts, and read or write files without ever touching your app runtime. Every eve agent receives one by default. The default `bash`, `read_file`, and `write_file` tools target it; you can also add the framework's `glob` and `grep` tools or access it from authored code.
...
With a sandbox enabled, eve writes byte-backed AI SDK `file` parts under
`/workspace/attachments` before the first model step and stores a sandbox
reference in session history. For provider calls, images up to 3 MiB and PDFs
up to 20 MiB are restored as bytes; other files become text references to their
sandbox paths. Unresolved remote URLs and legacy AI SDK `image` parts are not
staged.

When an agent [disables its sandbox](#disabling-the-sandbox), eve sends incoming
`file` parts to the model without staging them and preserves the inline payloads
in durable history. Provider limits still apply to the original file data.

Files created with `hostRuntimeFile()` are different: their bytes stay in the
trusted host application's file store whether the sandbox is enabled or
disabled. eve preserves only the `eve-attachment:` reference and display
metadata in durable history, skips `/workspace/attachments`, and asks the
active Host Runtime Provider for bytes immediately before each model request.
The transient bytes are not written back to the sandbox, session history,
events, or workflow snapshots.
...
Export `disableSandbox()` when an agent does not need filesystem or process
access and its attachments must reach the model without provisioning a sandbox:
...
The opt-out applies only to that agent node. eve removes the framework `bash`,
`read_file`, `write_file`, `glob`, and `grep` tools from the node, and
`ctx.getSandbox()` throws if authored runtime code calls it. Other framework
tools, authored tools, connections, and plain static skills remain available.

An agent with a disabled sandbox cannot declare `sandbox/workspace/**`, dynamic
skills, or static skill supporting files such as `references/`, `assets/`,
`scripts/`, or `defineSkill({ files })`. These capabilities need files in a live
sandbox, so eve rejects the combination during compilation. A flat Markdown
skill, a module-backed static skill without `files`, or a `SKILL.md`-only package
continues to load from its compiled Markdown.

Do not disable a sandbox in the middle of an existing durable session whose
history contains `eve-sandbox:` attachment references. Those references require
the original sandbox to restore their bytes. Re-enable the sandbox or start a
new session and attach the files again.
...
The parent and child then see the same files, processes, `/workspace`, and sandbox home. Nested children that also select `parent.sandbox` keep using the same owning sandbox rather than creating another one. The owning sandbox's `bootstrap` and `onSession` hooks remain authoritative; the child cannot add its own sandbox hooks or backend configuration. A child cannot select `parent.sandbox` when the resolved parent explicitly disables its sandbox.
...
The built-in `agent` self-delegation path shares the root agent's sandbox when it
is enabled. If the root exports `disableSandbox()`, its copies also run without
sandbox sharing metadata.
