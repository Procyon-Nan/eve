...
The sandbox is the agent's isolated bash environment: a filesystem rooted at `/workspace` where it can run shell commands, execute scripts, and read or write files without ever touching your app runtime. eve provides one by default for every agent node. The built-in `bash`, `read_file`, `write_file`, `glob`, and `grep` tools target it, and your authored code can too.
...
An agent that does not need shell commands, workspace files, or Sandbox-backed skills can opt out explicitly:
...
This is different from omitting the file. No `sandbox.ts` means “use the framework default”; exporting `disableSandbox()` means “register no sandbox for this agent node.” eve then skips backend selection, template prewarm, session creation, and optional `microsandbox` or `just-bash` installation.

With the sandbox disabled:

- Inline image and PDF `FilePart`s remain in the message and go directly to the configured model. eve does not write another copy or replace the payload with an `eve-sandbox:` URL. Context compaction uses a bounded, file-aware estimate instead of counting base64 as text, and the active turn remains intact until the model has received its files at least once.
- `bash`, `read_file`, `write_file`, `glob`, and `grep` are not exposed. Plain static skills containing only compiled instructions remain available through `load_skill`; packaged skills, inline skill files, and dynamic skills require a sandbox and are rejected during compilation.
- `agent/sandbox/workspace/**` is a compile-time conflict.
- `ctx.getSandbox()` and file reads through `ctx.getSkill()` are unavailable. Configure a sandbox before using those APIs.
- Ordinary authored and dynamic tools, `ask_question`, and subagent delegation remain available when they do not call Sandbox APIs.

The choice is node-local. A declared subagent does not inherit its parent's disabled state: give each subagent its own `sandbox.ts` if it should also opt out. Root-agent copies created by the built-in `agent` capability share the root configuration.

Disabling a sandbox does not weaken a channel's MIME type, size, authentication, or upload policy. It changes only what eve does after the channel has accepted a file.

Historical messages may already contain `eve-sandbox:` references. A disabled agent cannot read those references and fails with an instruction to re-enable the original sandbox or start a new session and attach the files again; opting out never starts a sandbox solely for historical compatibility.

Completed turns remain eligible for normal context compaction. Their image and PDF payloads may be replaced in the summary by a filename and media-type placeholder, so use a tool or a sandbox path when the model must reopen an old artifact. If the active turn itself exceeds the provider's real context limit, eve preserves its files and surfaces the provider error instead of silently dropping them.
