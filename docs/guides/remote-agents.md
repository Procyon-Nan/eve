...
The parent stream carries the same `subagent.called`, `action.result`, and `subagent.completed` events as local delegation. For a remote call, `subagent.called.data.message` preserves the exact message supplied by the caller, and `subagent.called.data.remote.url` records the target. The remote session receives eve's framework-formatted child prompt rather than the raw telemetry field.
