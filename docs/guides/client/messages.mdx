...
By default, eve stages inline attachment bytes in the current agent's sandbox and stores an `eve-sandbox:` reference in session history. An agent that explicitly exports `disableSandbox()` instead passes image and PDF file parts directly to the model without provisioning a backend or creating a second copy. This does not bypass the channel's MIME type, size, authentication, or upload checks. Existing sessions whose history already contains `eve-sandbox:` references must keep their original sandbox configuration or start a new session before switching to disabled mode.
