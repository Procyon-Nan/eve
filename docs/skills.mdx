...
Static skills that contain only instructions do not require a sandbox: `load_skill` returns their markdown directly from the compiled agent. Dynamic skills and static skills with supporting package files require a sandbox. An agent that exports `disableSandbox()` may use instruction-only static skills, but compilation rejects packaged skills, inline `files`, and dynamic skills for that node. Supporting files are normally available under `$HOME/.agents/skills/<skill>/`, with `/workspace/skills/<skill>/` as the fallback when `$HOME` is unavailable. Sibling references inside `SKILL.md`, such as `references/checklist.md`, are relative to the directory containing that specific `SKILL.md`.
...
Host runtime snapshots do not change these sandbox rules. If a host-backed agent disables its sandbox, provide database-backed skill discovery and loading as ordinary tools from `hostRuntimeTools`; do not return a dynamic `SkillPackageDefinition`. The host can lock the skill version alongside the model and instructions while its tool implements the model-visible loading contract.
