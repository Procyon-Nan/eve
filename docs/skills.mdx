...
Static skill Markdown does not require a sandbox: `load_skill` returns the instructions directly from the compiled agent. Dynamic skills and access to supporting package files require a sandbox. An agent that exports `disableSandbox()` can use flat Markdown skills, module-backed static skills without `files`, and `SKILL.md`-only packages, but compilation rejects dynamic skills and supporting `references/`, `assets/`, `scripts/`, or `defineSkill({ files })`. With a sandbox enabled, supporting files are available under `$HOME/.agents/skills/<skill>/`, with `/workspace/skills/<skill>/` as the fallback when `$HOME` is unavailable. Sibling references inside `SKILL.md`, such as `references/checklist.md`, are relative to the directory containing that specific `SKILL.md`.
...
Host runtime snapshots do not change the sandbox requirement for dynamic
skills. If a host-backed agent disables its sandbox, expose database-backed
procedure discovery and loading as ordinary tools from `hostRuntimeTools`
instead of returning a dynamic skill package. The host can lock the procedure
version with its model and instructions while its tools implement the loading
contract. See [Host runtime snapshots](./guides/dynamic-capabilities#host-runtime-snapshots).
