...
The copy uses the root's instructions, connections, auth, and sandbox selection. When the root sandbox is enabled, the copy shares it and its file writes are immediately visible to the root. When the root exports `disableSandbox()`, the copy also runs without sandbox access. It receives the same tools except for the root-only `agent` and `Workflow`, and starts with fresh conversation history and fresh state. To run independent tasks in parallel, emit multiple `agent` calls in one response; eve runs the batch concurrently and returns every result before the root continues. Give parallel children non-overlapping write scopes when they share a sandbox.
...
### Host-supplied specialist runtimes

A trusted server can supply a specialist's model, business instructions, and
ordinary dynamic tools as one locked runtime snapshot. Declare the same
provider on `defineDynamic` and the returned agent so eve can reject an
unauthorized specialist before its resolver runs:

```ts title="agent/subagents/reviewer/agent.ts"
import { defineAgent, defineDynamic, defineHostRuntime } from "eve";

const runtime = defineHostRuntime({ providerKind: "my-host" });
...
Host-runtime specialists are turn-scoped local subagents. They cannot select
their own model or compaction model. With agent messaging enabled, only a
task-owned call may use a host-runtime specialist; a plain conversation call
cannot leave one parked for later messages. The top-level root snapshot lists
authorized names in `delegatedSubagentNames`, and eve checks the authorization
again immediately before asking the provider for a child reference. A nested
session cannot pass that authorization to another host-runtime specialist.

The specialist directory still owns its static instructions, sandbox choice,
connections, and other authored slots. Use `hostRuntimeInstructions` and
`hostRuntimeTools` in dynamic instruction and tool slots to add the host
snapshot without exposing its opaque reference. A built-in `agent` copy can
reuse the root turn's snapshot, but it receives no specialist authorization
and does not own a separate reference.

The provider's `createSpecialistReference` operation must be idempotent for the
parent session, parent turn, call ID, and subagent name. eve retains that child
reference through task and Workflow dispatch, then calls the provider's
optional `release` only after the owning root turn or complete specialist call
has committed a terminal outcome: `completed`, `failed`, `cancelled`, or the
specialist-only `start_failed`. Release is a best-effort, idempotent
notification; a release failure does not roll back the terminal eve state. See
[Host runtime snapshots](./guides/dynamic-capabilities#host-runtime-snapshots)
for provider registration and root capability setup.
...
| Sandbox      | Shares the root selection     | Own `sandbox/`, else framework default |
...
The root built-in `agent` tool is the exception. Its children share the root's sandbox when enabled and use the root's sandbox opt-out when disabled because they are copies of the same agent.
...
Each delegated subagent spins up its own child session and stream. The parent stream carries the control-plane events `subagent.called` and `subagent.completed`, plus interactive `input.requested`, `authorization.required`, and `authorization.completed` events proxied from descendants so the root channel can prompt the user. `subagent.called.data.message` is the exact `message` tool input for that call, while `data.childSessionId` identifies the child stream to subscribe to at `GET /eve/v1/session/:childSessionId/stream`. The child itself receives a framework-formatted prompt containing the delegation message.

A local subagent child stream's first and only `session.started` event includes `data.invocation` with the immediate parent's session ID, turn ID, tool-call ID, and the invoked subagent name. eve generates this lineage metadata at runtime; it is not model output. Root-session `session.started` events do not include `invocation`.
