...
| `sandbox.ts` or `sandbox/sandbox.ts`                    | Sandbox selection for this agent node       | Yes       | Use `defineSandbox(...)` to configure a sandbox or `disableSandbox()` to opt out. The framework default applies when neither file is authored. The folder layout pairs only with an enabled `sandbox/workspace/**`.   |
