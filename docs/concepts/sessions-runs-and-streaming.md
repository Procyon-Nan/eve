...
| `subagent.called`         | A subagent was delegated; carries `childSessionId` and the exact original delegation `message`.                  |
...
A delegated subagent publishes progress on its own child-session stream. The parent emits `subagent.called` with a `childSessionId`, which a client uses to attach, and the exact original delegation text in `data.message`. The child's `message.received.data.message` is synthesized model input and is not a stable source for recovering that text.
...
Conversation model calls make at most three attempts for any model-call error, including authentication, invalid-model, other `4xx`, rate-limit, timeout, and `5xx` responses. If all attempts fail, the stream ends the turn with `step.failed`, `turn.failed`, and `session.waiting`; the continuation token remains valid, and the next user message resolves the model configuration again. Any `message.appended` output without a matching `message.completed` belongs to the failed turn and should be discarded as an unfinished draft when `turn.failed` arrives. Cancellation does not use the retry budget. Task and scheduled runs retain terminal failure semantics because no user may be available to continue them.
